...
def update(u: np.ndarray,
    ...
    JI = condense(J, D=D, expand=False)
    M.matvec = partial(solver_direct_scipy(), JI)
...
x[I] = sol.x
print('L_inf residual:', np.linalg.norm(residual(x[I]), np.inf))
