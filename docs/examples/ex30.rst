...
A classical iterative procedure for the Stokes equation is the Uzawa congugate gradient method [GLOWINSKI-PIRONNEAU]_ [BRAESS]_ in which an auxiliary operator is formed which maps a guessed pressure field to the divergence of the velocity field which satisfies the momentum equation with that pressure.

.. [BRAESS] Braess, D. (2001). *Finite Elements.*_ Cambridge University Press, §IV.5.1
