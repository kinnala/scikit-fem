...
In this example, the :ref:`forcedconvection` example is extended to conjugate heat transfer by giving a finite thickness and thermal conductivity to one of the walls.
...
Here Pe is the dimensionless Péclet number and :math:`k` the ratio of the local thermal conductivity to that of the fluid.

This fully developed solution applies throughout the domain if it is specified along the fluid and solid inlet :math:`x=0`, if the ceiling is insulated :math:`\partial T/\partial y = 0` on :math:`y = 1`, if the floor is uniformly heated :math:`k\partial t/\partial y = 1` on :math:`y = -2`, and if the longitudinal gradient is applied as a uniform Neumann condition on the outlet, :math:`k\partial T/\partial x = 3k/4\mathrm{Pe}` on :math:`x = \ell`.
