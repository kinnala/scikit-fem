...
class ElementTriN3(ElementHcurl):
    ...
    def gbasis(self, mapping, X, i, tind=None):
        """Covariant Piola transformation. Overridden to allow for higher order"""
        ...
        if i < 9:
            ...
            local_idx = i % 3
            if edge_idx in [0, 1]:
                if local_idx == 0: target_swap = i + 2
                elif local_idx == 2: target_swap = i - 2
        ...
        def get_lbasis_fixed(idx):
            ...
            if 6 <= idx <= 8:
                return -p, -dp # Edge 2 vectors point down in definition, but reference points Up.
            return p, dp
    ...
    def lbasis(self, X, i):
        ...
        if i == 0:
            phi = np.array([
                -8.0*x2*y + 8.0*x2 - 16.0*x*y2 + 26.0*xy - 10.0*x - 8.0*y**3 + 18.0*y2 - 13.0*y + 3.0,
            ...
            phi = np.array([
                16.0*x2*y - 16.0*x2 + 16.0*x*y2 - 32.0*xy + 16.0*x - 4.0*y2 + 7.0*y - 3.0,
            ...
            phi = np.array([
                ...
                -16.0*x2*y + 4.0*x2 - 16.0*x*y2 + 32.0*xy - 7.0*x + 16.0*y2 - 16.0*y + 3.0
            ])
            ...
            phi = np.array([
                ...
                8.0*x**3 + 16.0*x2*y - 18.0*x2 + 8.0*x*y2 - 26.0*xy + 13.0*x - 8.0*y2 + 10.0*y - 3.0
            ])
            dphi = 32.0*x2 + 64.0*xy - 46.0*x + 32.0*y2 - 46.0*y + 16.0
