...
@pytest.mark.parametrize(
    ...
    [
        ...
        (MeshQuad, ElementQuad1()),
        (MeshQuad, ElementQuad2()),
        (MeshTet, ElementTetP1()),
        (MeshTet, ElementTetP2()),
        (MeshHex, ElementHex1()),
        (MeshHex, ElementHex2()),
    ],
)
def test_trace(mtype, e1):

    m = mtype().refined(3)

    # use the boundary where last coordinate is zero
    fbasis = FacetBasis(m, e1, facets=m.facets_satisfying(lambda x: x[-1] == 0.0))
    basis = CellBasis(m, e1)
    xfun = basis.project(lambda x: x[0])
    y = fbasis.project(basis.interpolator(xfun))
    ...
    assert_almost_equal(integ.assemble(fbasis, y=fbasis.interpolate(y)), .5)
...
def test_subdomain_facet_assembly():
    ...
    u0_dofs = np.hstack((cbasis.get_dofs(), cbasis.get_dofs(elements=subdomain)))
